...
export default function Navbar() {
  ...
  useEffect(() => { if (dark === null) return; document.documentElement.dataset.theme = dark ? 'dark' : 'light'; localStorage.setItem('theme', dark ? 'dark' : 'light') }, [dark])
  return <header className="site-header fixed inset-x-0 top-0 z-50">
    <div className="container flex h-full items-center justify-between gap-4">
      <a href="#home" className="flex shrink-0 items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[color:var(--text-strong)] text-sm font-bold text-[color:var(--bg)]">SJ</span><span className="hidden text-sm font-semibold text-strong sm:block">Sharad Jha</span></a>
      <nav aria-label="Main navigation" className="hidden flex-1 items-center justify-center gap-7 md:flex lg:gap-9">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        <button type="button" aria-label={dark ? 'Switch to light theme' : 'Switch to dark theme'} aria-pressed={dark === false} onClick={() => setDark((value) => !value)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[color:var(--border)] bg-[color:var(--surface)] text-body hover:bg-[color:var(--surface-hover)]">{dark ? <Sun size={17}/> : <Moon size={17}/>}</button>
